...
/*-------------------- myfuntion ------------------*/
		function readExcel($file)
		{
			...
			for($i=1;$i<=$col;$i++)
			{
			array_push($fields,$data->val(1,$i));
			}
			for($t=2;$t<=$row;$t++)
			{
				$datav[$t-2] = array();
				for($i=1;$i<=$col;$i++)
				{
					array_push($datav[$t-2],preg_replace("~[^a-z0-9A-Z,&()'/ .:+-]~",'',$data->val($t,$i)));
				}
			}
			$returndata = new stdClass;
			$returndata -> rows = $row-1;
			$returndata -> columns = $col;
			$returndata -> fields = $fields;
			$returndata -> data = $datav;
			return $returndata;
